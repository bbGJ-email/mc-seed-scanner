...
| 7. AI 智能推荐持续探测 | 持续扫描（挂机模式）+ 智能推荐高分种子 + OpenAI 兼容 API 实时生成种子介绍 | `mcss/ai_intro.py`、`mcss/task_manager.py` |
...
### AI 智能推荐 / 持续扫描（v1.1 新增）

- **持续扫描（挂机模式）**：勾选「持续扫描」后，扫描区间自动扩展为超大范围，可长时间挂机自动连续扫描，
  直到手动停止；断点续扫自动兜底，重启程序后从上次位置继续。
- **智能推荐**：扫描命中入库后，按「评分阈值 + 最低等级」规则自动挑选高价值种子，实时进入「智能推荐」页。
- **AI 种子介绍**：对推荐种子调用任意 **OpenAI 兼容接口**（OpenAI 官方 / DeepSeek / Moonshot / 通义千问 / 本地 vLLM / Ollama 等）
  实时生成中文种子介绍（出生点、结构坐标、地形亮点、适合玩法），生成状态与结果可随时查看，支持手动重新生成。

**自动更新（v1.1 新增）**

程序左侧「自动更新」面板支持从 GitHub Release 自动获取新版源码包并重建：
勾选「启动时自动检查更新」后，每次启动后台比对版本；或点「立即检查更新」。
发现新版 → 确认后自动下载源码包 → 解压覆盖（保留 `data/`、`tools/`、`venv/`）→
自动运行 `build_windows.bat` 重建 EXE。发布端维护 `scripts/make_release.py` 建 Release 即可。

**AI 配置（左侧「AI 智能推荐 / 持续扫描」面板）**：

| 配置项 | 说明 |
|---|---|
| API 地址 | OpenAI 兼容端点，如 `https://api.openai.com/v1`、`https://api.deepseek.com/v1` |
| API Key | 对应平台的密钥 |
| 模型 | 如 `gpt-4o-mini`、`deepseek-chat`、`qwen-plus` |
| 介绍评分阈值 | 评分达到该值才自动生成介绍（默认 75） |
| 最低等级 | 达到该等级（S+/S/A/B/C）即自动生成（默认 S） |

配置保存在程序数据目录 `data/ai_config.json`（EXE 运行时为 EXE 旁 `data\ai_config.json`）。
...
> **运行期数据目录**：`data/` 位于程序基目录下——源码运行时为项目根；打包 EXE 运行时为
> EXE 所在目录（数据库、日志、断点、命中缓存全部落在那里，重启不丢失）。
```
